...
spec = [('sigma', UniTuple(float64,2)),
            ...
            ('pvalid', float64),
            ('task_prior', float64),
            ('sample_actions', ListType(ListType(unicode_type))),
            ('update_rule', unicode_type),
            ('inv_temp', float64),
            ('v', float64[:,:,:])]

@jitclass(spec)
class Bern3Diff:
    def __init__(self, sigma=(0.6, 0.6), theta=0.6, pvalid=1.0, task_prior=0.5, cost=-0.001, \
                 update_rule='max', inv_temp=1e3, sample_actions=List([List(['x0']), List(['x1']), List(['z'])]), state_range=50):
        ...
        self.update_rule, self.inv_temp = update_rule, inv_temp
        ...
        self.cue_log_odds = log(pvalid*task_prior + (1-pvalid)*(1-task_prior)) - \
            log((1-pvalid)*task_prior + pvalid*(1-task_prior))
        ...
        optimize_value(self, maxiter=1000)

    # core computations 
    @staticmethod
    def prob_h(log_odds, x, offset=0.0):
        return 1.0 / (1.0 + exp(-(x*log_odds + offset)))
    
    def prob_h1(self, x, task=0):
        return self.prob_h(self.x_log_odds[task], x[task])
    
    def prob_h0(self, x, task=0):
        return self.prob_h(self.x_log_odds[task], -x[task])
    
    def prob_c1(self, z):
        return self.prob_h(self.z_log_odds, z, self.cue_log_odds)
    ...
    def state_sample_value_vector(self, x, z):
        v = np.empty(len(self.sample_actions), float64)
        for ind, a in enumerate(self.sample_actions):
            v[ind] = self.state_sample_value(x, z, a)
        return v
    
    def state_action_value_vector(self, x, z):
        nsa = len(self.sample_actions)
        v = np.empty(nsa + 2, float64)
        v_sample = self.state_sample_value_vector(x, z)
        v[:nsa] = v_sample
        v[nsa] = self.state_terminate_value_h0(x, z)
        v[nsa+1] = self.state_terminate_value_h1(x, z)
        return v

    def state_value_update(self, x, z):
        ind = self.index_of_state(x, z)
        v = self.state_action_value_vector(x, z)
        match self.update_rule:
            case 'max':
                new_value = np.max(v)
            case 'softmax':
                new_value = np.dot(v, softmax_prob(v*self.inv_temp))
...
@njit
def choice(p):
    return np.argmax(np.cumsum(p) > random())
